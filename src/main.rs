...
mod app {
    ...
    pub(crate) enum Error {
        ...
        Io(io::Error),
        Canvas(canvas::Error),
        Parse(export::ParseError),
        Save(export::SaveError),
    }
    ...
    impl fmt::Display for Error {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            let msg = match self {
                ...
                Error::Canvas(_) => "canvas operation failed",
    ...
    impl From<canvas::Error> for Error {
        fn from(canvas_error: canvas::Error) -> Self {
            Error::Canvas(canvas_error)
        }
    }
    ...
    impl From<export::ParseError> for Error {
        fn from(parse_error: export::ParseError) -> Self {
            Error::Parse(parse_error)
        }
    }

    impl From<export::SaveError> for Error {
        fn from(save_error: export::SaveError) -> Self {
            ...
            match c? {
                ...
                Event::Key(Key::Ctrl('s')) => {
                    ...
                    if let Some(name) = &save_file_name {
                        export::save_as(&blueprint?, &name)?;
                    } else {
                        save_file_name = Some(export::save(&blueprint?)?);
                    }
                }
